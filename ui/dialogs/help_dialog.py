...
class HelpDialog(QDialog):
    ...
    а также описывает новые возможности версии 0.21.0 (окно просмотра, пакетное редактирование,
    горячие клавиши, умная сборка названия и т.д.).
    """
    ...
    def _init_ui(self):
        ...
        # === Вкладка 4: Визуальные сигналы + новые возможности ===
        signals_tab = QWidget()
        ...
        tabs.addTab(signals_tab, "🎨 Цвета и фишки")
    ...
    def _get_workflow_text(self):
        ...
        <h3>Шаг 3: Эксплуатация (новые возможности)</h3>
        <ul>
            ...
            <li>👁️ <b>Двойной клик</b> по строке — откроется окно просмотра детали (фото, все свойства, печать).</li>
            <li>✏️ <b>Выделите несколько деталей</b> (Ctrl+клик) и нажмите «Пакетное редактирование» — массовое изменение полей.</li>
            <li>🧩 <b>Умная сборка названия</b>: введите число, выберите единицу (Ом, мкФ), нажмите «Собрать название» — программа сама сформирует имя.</li>
            <li>⌨️ <b>Горячие клавиши</b>: Ctrl+N (новая), Ctrl+E (редактировать), Del (удалить), Ctrl+F (поиск), F5 (обновить).</li>
        </ul>
        """
...
🏠 <b>Место</b> (Где? Например: "Гараж", "Офис")
   └── 📦 <b>Контейнер</b> (В чём? Например: "Шкаф 1", "Ящик с SMD")
       └── 📦 <b>Полка</b> (Где внутри? Например: "Полка 2")
        ...
        <h3>🆕 Особенности для конденсаторов</h3>
        <p>Для конденсаторов доступны поля <b>диаметр (мм), высота (мм), шаг выводов (мм), толщина выводов (мм)</b>. Они автоматически отображаются в колонке «Корпус» в таблице.</p>
        """

    def _get_signals_text(self):
        return """
        <h2>🎨 Язык цветов и новые фишки</h2>
        ...
        <h3>3. Дополнительные визуальные подсказки</h3>
        <ul>
            <li>📷 <b>Иконка фото</b> в колонке «Корпус» означает, что у детали есть изображение.</li>
            <li>📏 <b>Размеры конденсаторов</b> (⌀10×17мм, шаг 5мм) отображаются прямо в колонке «Корпус».</li>
        </ul>
        
        <h3>4. Новые возможности управления</h3>
        <ul>
            <li>👁️ <b>Окно просмотра</b> (двойной клик): вся информация о детали, увеличение фото колёсиком мыши, печать карточки.</li>
            <li>✏️ <b>Пакетное редактирование</b>: выделите несколько строк (Ctrl+клик) → кнопка «Пакетное редактирование» → массовое изменение полей.</li>
            <li>🧩 <b>Умная сборка названия</b>: в диалоге редактирования введите число, выберите единицу измерения и нажмите «Собрать название» — программа сама составит имя.</li>
            <li>⌨️ <b>Горячие клавиши</b>: ускоряют работу (список горячих клавиш см. в меню программы).</li>
            <li>📋 <b>Выпадающие списки</b> для «Тип детали» и «Производитель» — можно добавлять новые значения, они запоминаются.</li>
            <li>📅 <b>Кнопка «Сегодня»</b> для быстрой установки даты ревизии.</li>
            <li>💾 <b>Сохранение окна</b>: программа запоминает размер, позицию и развёрнутое состояние окна.</li>
        </ul>
        
        <p style="color: gray; font-size: 10pt; text-align: right;">Система визуальных подсказок и возможностей v0.21.0</p>
        """