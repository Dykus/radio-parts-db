...
class PartDialog(QDialog):
    ...
    def _get_category_id_from_path(self, category_path):
        if not category_path or not self.db: return None
        ...
        if len(path_parts) == 1:
            for cat_id, name, parent_id in cats:
                if name == target_name and parent_id in (None, 0): return cat_id
            return None
            
        item_map = {c[0]: (c[1], c[2]) for c in cats}
        for cat_id, name, parent_id in cats:
            if name != target_name: continue
            current_path = []; current_id = cat_id
            while current_id and current_id in item_map:
                ...
                if len(current_path) == len(path_parts):
                    if current_path == path_parts: return cat_id
                    break
                if current_parent_id in (None, 0): break
    ...
    def _build_category_path(self, cat_id, cats):
        ...
        item_map = {c[0]: (c[1], c[2]) for c in cats}
        path_parts = []; current_id = cat_id
        while current_id and current_id in item_map:
            name, parent_id = item_map[current_id]
            path_parts.insert(0, name)
            if parent_id in (None, 0): break
    ...
    def get_location_string(self):
        return ' / '.join([p.strip() for p in [
            self.location_place_combo.currentText(), self.location_container_combo.currentText(),
            self.location_shelf_combo.currentText(), self.location_section_combo.currentText()
        ] if p.strip()])
